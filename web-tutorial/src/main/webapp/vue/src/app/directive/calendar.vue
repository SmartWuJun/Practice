...
//                      console.log(this.lunar(this.year,this.month,i));
                    ...
                    /*  for()*/
                      let active = false;
                      this.multipleToday.forEach((v,index)=>{
                          if(_this.year+"-"+(_this.month+1) +"-"+i==v){
                              active=true;
                          }
                      });
                      if(active){
            ...
            if (this.range) {
                ...
                if(this.multiple){
                  ...
                  if(this.days[k1][k2].selected==false){
                    this.multipleToday.push(this.year+"-"+(this.month+1)+"-"+this.days[k1][k2].day);
                  }else {
                    this.multipleToday.forEach((v,index)=>{
                      if(_this.year+"-"+(_this.month+1)+"-"+_this.days[k1][k2].day==v){
                        this.multipleToday.splice(index,1)
                      }
                    });
                  }
              ...
              if(!this.multiple){
                ...
                this.$emit('select',this.multipleToday)
              }
            }
        },
        // 日期补零
    ...
    watch: {
      'multipleToday': function () {
        if(this.multipleToday.length==0){
          this.render(this.year, this.month)
        }
      }
    }
