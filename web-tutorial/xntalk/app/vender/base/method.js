(function (window) {
     ...
     * 消息监听器
     */
    window.messageWatcher = function(data) {

        //获取页面最外层控制器中的scope
        var scope = angular.element('#bady_controller').scope();
        scope.$apply(function() {

            if(scope.messages && 5<=scope.messages.length) {
                scope.messages.pop();
            }
...
(function (Date, undefined) {
    ...
    var moment, round = Math.round, languages = {}, hasModule = (typeof module !== 'undefined'),
        paramsToParse = 'months|monthsShort|monthsParse|weekdays|weekdaysShort|longDateFormat|calendar|relativeTime|ordinal|meridiem'.split('|'), i,
        jsonRegex = /^\/?Date\((\-?\d+)/i,
    ...
    moment = function (input, format) {
        ...
        if (input && input._d instanceof Date) {
            ...
            date = input === undefined ? new Date() : matched ? new Date(+matched[1]) : input instanceof Date ? input :
                isArray(input) ? dateFromArray(input) : typeof input === 'string' ? makeDateFromString(input) : new Date(input);
        }
    ...
    moment.humanizeDuration = function (num, type, withSuffix) {
        var difference = +num, rel = moment.relativeTime, output;
    ...
    /**
     * long类型的时间转换成yyyy-MM-dd HH:mm格式
     * @returns {*}
     */
    ...
    /**
     * string类型的时间转换成yyyy-MM-dd HH:mm格式
     * @returns {*}
     */
    ...
    /**
     * long类型的时间转换成yyyy-MM-dd HH:mm:ss格式
     * @returns {*}
     */
    ...
    /**
     * string类型的时间转换成yyyy-MM-dd HH:mm:ss格式
     * @returns {*}
     */
