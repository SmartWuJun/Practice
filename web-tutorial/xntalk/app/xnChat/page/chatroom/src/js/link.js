...
var LinkRoom = function (ctr,data) {
    ...
    var doLink = function(data){
        that.room = new Chatroom({
            appKey: data.appKey,//测试
            account: data.account,
            token: data.token,
            //不传nick跟avatar 默认im的nick avatar
            chatroomNick:data.chatroomNick,
            chatroomAvatar:data.chatroomAvatar,
            chatroomId: data.id,
            ...
            onconnect:function(msg) {
                ...
                // that.getChatroomMembers(false,0,20,cbGetChatroomMembers.bind(that));
                ...
                if (error) {
                    ...
                    case 'kicked':
                        if(error.reason==="managerKick"){
                            localStorage.setItem("kickReason",'你已被管理员移出|'+that.data.roomInfo.name);   
                        }else if(error.reason==="blacked"){
                            localStorage.setItem("kickReason",'你已被管理员拉入黑名单，不能再进入|'+that.data.roomInfo.name); 
                        }
