...
var myTeam = {
	...
	addEvent: function() {
		...
		this.$createTeamContainer.delegate('.main_icon-close', 'click', this.closeDialog.bind(this));
	...
	editTeamName: function() {
		...
		if ($this.hasClass('owner')) {
			...
			$this.parent('.wrap').find("input").attr("placeholder",name);
	...
	showUserList: function(type,teamType) {
        ...
        if(type == 1)
        {
            ...
            controllerScope.teamId =  $('#j-userList .first').data('team-id').toString();
            controllerScope.eleCloudFileSelectisShow = true;
            return;
        }
		var that = this;
	...
	showTeamInfo: function(o) {
		var $this = $(o),
	 		that = this;
		this.$teamInfoContainer.load('/xntalk/team-info.html', function() {
			// 获取群成员
            ...
            try
            {
                if(JSON.parse(teamInfo.serverCustom).type == "tenant")
                {
                    isTenant = true;
                }
            }
