    ...
    switch (msg.type) {
        ...
        case 'custom':
            ...
            if(content.type===1){
                ...
                {
                    content.data.time = getLocalTime(content.data.time);
                }
                if (!!content.time == true)
                {
                    content.time = getLocalTime(content.time);
                }



              //  str = '<div class="xn-msg-custom"><span>' + content.data.applicationType + '</span> </div> <div class="j-mbox left"> <div class="cell-title ng-binding">' + content.data.title + '</div> <br/> <img class="a-img" ng-src="' + content.data.pictureUrl + '" alt="' + content.data.title + '" src="' + content.data.pictureUrl + '" <div class="cell-info"> <p class="cell-author ng-binding">' + content.data.summary + '</p>  <p class="cell-author ng-binding">' + content.data.author + '</p> <p class="cell-author ng-binding  xn-bottom-line"> 时间: ' + getLocalTime(content.data.time) + '</p></div> <a href="' + content.data.url + '"> <span class="cell-author ng-binding"> 查看详情 </span> </a></div>';
                // str = sentStr+'一条[自定义]消息，请到手机或电脑客户端查看';

                str = $("#customMsgRender").render(content);
...
function getRealName(account,cache){
    cache = cache||yunXin.cache;
    var user = cache.getUserById(account);
    try {
        if(!!user.custom == true &&  !!JSON.parse(user.custom).realName && JSON.parse(user.custom).realName.length > 0 )
        {
            return JSON.parse(user.custom).realName;
        }
