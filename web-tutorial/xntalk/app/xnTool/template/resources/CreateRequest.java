/*
 * @(#){{ table.nameNoPrefix | pascalcase }}CreateRequest.java
 *
...
import com.{{ setting.companyCode | lower}}.framework.base.BaseRequest;
...
/**
 * 创建{{ table.meaning }}的请求.
 * @author {{ setting.developerName}}
 */
public class {{ table.nameNoPrefix | pascalcase }}CreateRequest extends BaseRequest {
    {% for column in table.columnsNoIdAndTenantId %}
    /**
     * {{ column.meaning  }}
     * {{ column.description  }}
     */
    {% if column.type == "String" %}{% if column.isRequired == "true" %}@NotBlank(message = "{{ column.meaning  }}不能为空"){% endif %}
    @Length(min={{column.min}}, max={{column.max}}, message = "{{ column.meaning  }}长度不合法"){% else %}{% if column.isRequired == "true" %}@NotNull(message = "{{ column.meaning  }}不能为空"){% endif %}{% endif %}
