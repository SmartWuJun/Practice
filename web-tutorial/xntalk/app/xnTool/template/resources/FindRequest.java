/*
 * @(#){{ table.nameNoPrefix | pascalcase }}FindRequest.java
 *
...
import com.{{ setting.companyCode | lower}}.framework.base.BaseFindRequest;

/**
 * 高级查询{{ table.meaning }}的请求.
 * @author {{ setting.developerName}}
 */
public class {{ table.nameNoPrefix | pascalcase }}FindRequest extends BaseFindRequest { {% for column in table.columns %}{% if column.isQueryable == "true" %}

    /**
     * {{ column.meaning  }}
     * {{ column.description  }}
     */
    private {{ column.java }} {{ column.name | camelcase }};{% endif %}{% endfor %}{% for column in table.columns %}{% if column.isQueryable == "true" %}
